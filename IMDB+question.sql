...
/* In above solution, id in each case statement has been used as a counter to count the number of null values. Whenever a value
...
-- Second part alternate solution
SELECT 
...
SELECT 
    COUNT(*) AS total_movies_with_only_one_genre
FROM
    (SELECT 
        movie_id
    FROM
...
SELECT title,
	   avg_rating,
       RANK() OVER(ORDER BY title DESC) AS movie_rank
FROM movie AS m 
...
SELECT 
    title, avg_rating, median_rating, genre
FROM
    movie AS m
        INNER JOIN
    genre AS g ON m.id = g.movie_id
...
/* by adding median_rating column to the above table, we can see that the movie "The Gambinos" median rating is 10, 
but the avg rating is quite low i.e., 8.4, this is the only inference we can make from this. */
...
FROM
    names
WHERE
    name IS NULL;            -- there are no nulls in name column
...
FROM
    names
WHERE
    height IS NULL;         -- there are 17335 null values in height column
...
FROM
    names
WHERE
    date_of_birth IS NULL;     -- there are 13431 null values in date_of_birth column
...
FROM
    names
WHERE
    known_for_movies IS NULL;   -- there are 15226 null values in known_for_movies column
...
|	.				|		.			|			.		  |
+-------------------+-------------------+---------------------+*/
-- Type your code below:            --total votes- sum(total_votes)

SELECT production_company,
       SUM(total_votes) AS vote_count,
       RANK() OVER (ORDER BY production_company DESC) AS prod_comp_rank
FROM movie AS m
INNER JOIN ratings AS r ON m.id = r.movie_id
GROUP BY production_company 
...
-- the genre-wise running total and moving average of the average movie duration are shown in the output.
...
/* Inference: We can see that the top three genres are comedy, drama and thriller 
and the five highest-grossing movies of each year that belong to the top three genres are listed in the output. */
...
/* Inference: We can infer that the top two production  houses that have produced the highest 
number of hits (median rating >= 8) among multilingual movies are Star Cinema and Twentieth Century Fox */
...
/* Inference: From the query above, we can infer that the top three actresses based on number of superhit movies 
i.e., who have avg_rating above 8 in drama genre are Sangeetha Bhat, Fatmire Sahiti and Adriana Matoshi.
...
/* Inference: The above table gives us the information about the directors of the movies with their ids, names, avg_rating,
total_votes, no. of movies they have directed etc.*/